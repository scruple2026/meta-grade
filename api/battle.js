...
function normalizeNotes(notes) {
  ...
  return {
    ...
    sensing: cleanText(value.sensing, MAX_TEXT_LENGTH),
    tactics: cleanText(value.tactics, MAX_TEXT_LENGTH),
...
function buildSystemPrompt() {
  return [
    ...
    "必须完整阅读并使用 notes.penetration、notes.resistance、notes.sensing、notes.tactics、notes.special、notes.weakness、notes.setting、notes.basis 和 notes.timeline；这些解释项不能因为主面板已简写而省略。",
    "必须默认考虑 notes 中的攻击性质、防御抗性、感知索敌、战术智商、特殊权能、领域、封印、空间、灵魂、一次性、外源、仪式、装备等，但只能按 notes 和峰值标签解释；条件不明时必须写入 caveats。",
