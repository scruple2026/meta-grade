...
- AI 对战链接会记录当前左右角色、时间线和输出风格，方便复制分享；生成中可以从前端取消请求，同一角色同一时间线不会发起无意义调用。页面会读取 `/api/battle` 的 GET 状态展示当前模型、Key 配置状态和 Chat fallback 能力，并在流式生成时显示 Responses / fallback 路径。生成结果可复制成纯文本，包含角色、时间线、裁定、关键因素、阶段过程、限制和分享链接。
