...
5. Keep user-facing classification fields clean. Do not put selection or maintenance labels such as `热门战力`, `高讨论度`, `待审核`, or source status wording into `category`, `affiliation`, or `grade`. Use stable role terms such as `主角团`, `核心反派`, `最终 boss`, `核心战力`, `四皇`, or `顶级剑士`; for high-risk main ranks, prefer `核心战力`/`核心反派`. Treat `grade` as a descriptive `身份 / 能力` field, not as a filter taxonomy: it may contain narrower titles such as `总队长`, `队长`, `零番队`, `传说忍者`, or a signature ability, but it should not be exposed as a search filter or sort mode because it tends to become one option per character.
